...
import { useHqStock, useHqStockTxs } from "../hooks/useHqInventory"
import type { HqStockTxResponse } from "../types/dto"
import { HQ_STOCK_EVENT_LABEL, type HqStockEvent } from "../types/enums"
import { StockAdjustModal } from "./StockAdjustModal"

type HqInventoryDrawerProps = {
...
const eventOptions = [
  ...
  ...Object.entries(HQ_STOCK_EVENT_LABEL).map(([value, label]) => ({
    value,
    label,
  })),
]

const columns: TableProps<HqStockTxResponse>["columns"] = [
  {
    title: "유형",
    dataIndex: "event",
    key: "event",
    render: (event: HqStockEvent) => HQ_STOCK_EVENT_LABEL[event],
  },
  {
    title: "수량",
    dataIndex: "quantity",
...
export function HqInventoryDrawer({
  stockId,
  open,
  onClose,
}: HqInventoryDrawerProps) {
  const [event, setEvent] = useState<HqStockEvent | "all">("all")
  const [start, setStart] = useState<string | undefined>()
  const [end, setEnd] = useState<string | undefined>()
  const [page, setPage] = useState(0) // 현재 페이지 0-indexed

  const { data: stock, isLoading: isStockLoading } = useHqStock(stockId)
  const { data: stockTxs, isLoading: isStockTxsLoading } = useHqStockTxs(
    stockId,
    {
